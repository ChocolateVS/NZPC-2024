...
def evaluateExpression(exp: str):
  try: 
    if exp[0] == "(":                         # Checks if the expression has a bracket
      ...
      raise Exception                         # If the expression doesn't have a bracket its a integer
  except:
  ...
  # Here means we are in an expression
  [p, exp] = evaluateExpression(exp)      # Will parse p from the expression
  [e1, exp] = evaluateExpression(exp)     # Will attempt to evaluate the expression at e1
  [e2, _] = evaluateExpression(exp)       # Will attempt to evaluate the remaining part of the expression
...
while (True):
  ...
  if expression  == "()":
    break
  print(evaluateExpression(expression)[0])